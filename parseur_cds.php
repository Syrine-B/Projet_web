...
// cette page est un parseur, il permet a l'administrateur d'ajouter une ou plusieurs sequence codante a partir de fichier fasta
    ...
    while (!feof($fichier)){
      ...
      if(preg_match("#^>#", $ligne)){
        ...
        if(!empty($sequence)){
          ...
          $req_fin = $bdd->prepare("UPDATE SEQUENCE SET seq_dna = ? WHERE id_seq = ?");
          $test = $req_fin->execute(array($sequence,$id_sequence));
        }
        ...
        $id_sequence = str_replace(" cds chromosome","", $id_seq);
        $id_sequence = str_replace(" cds plasmid","", $id_sequence);
        $id_genome = $tete[1];
        $debut_cds = $tete[3];
        $fin_cds = $tete[4];
        ...
        $taille_cds = intval($fin_cds) - intval($debut_cds);
        ...
        if(empty($existe)){ //si la sequence n'existe pas
          ...
          $req_seq->execute(array($id_sequence,$id_genome));
        }
        else{
...
// ne pas oublier de joindre id_annotation de annotation a id_annotation de sequence
        ...
        if(empty($existe_annot)){
          ...
          $req_annot->execute(array($gene_nom, $gene_biotype, $transcript, $gene_symbole, $description, $taille_cds, $debut_cds, $fin_cds, $sens));
          ...
          $req_annot->execute(array($gene_nom, $gene_biotype, $transcript, $gene_symbole, $description, $taille_cds, $debut_cds, $fin_cds, $sens,$id_sequence));
